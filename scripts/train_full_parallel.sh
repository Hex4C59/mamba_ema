#!/bin/bash
# 完整训练 5 fold（50 epochs）

echo "========================================"
echo "并行训练 5 Fold - 完整训练（50 epochs）"
echo "========================================"
echo ""
echo "配置:"
echo "  - Epochs: 50"
...
# 询问是否继续
read -p "是否开始完整训练（50 epochs）? (y/n): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]
then
    echo "取消训练"
...
CUDA_VISIBLE_DEVICES=0 uv run python src/train.py \
    --config configs/finetune_full.yaml --fold 1 --gpu 0 \
    > logs/finetune_full/fold1.log 2>&1 &
PID1=$!
echo "Fold 1 启动 (PID: $PID1, GPU: 0)"

CUDA_VISIBLE_DEVICES=1 uv run python src/train.py \
    --config configs/finetune_full.yaml --fold 2 --gpu 0 \
    > logs/finetune_full/fold2.log 2>&1 &
PID2=$!
echo "Fold 2 启动 (PID: $PID2, GPU: 1)"

CUDA_VISIBLE_DEVICES=2 uv run python src/train.py \
    --config configs/finetune_full.yaml --fold 3 --gpu 0 \
    > logs/finetune_full/fold3.log 2>&1 &
PID3=$!
echo "Fold 3 启动 (PID: $PID3, GPU: 2)"

CUDA_VISIBLE_DEVICES=3 uv run python src/train.py \
    --config configs/finetune_full.yaml --fold 4 --gpu 0 \
    > logs/finetune_full/fold4.log 2>&1 &
PID4=$!
echo "Fold 4 启动 (PID: $PID4, GPU: 3)"

CUDA_VISIBLE_DEVICES=4 uv run python src/train.py \
    --config configs/finetune_full.yaml --fold 5 --gpu 0 \
    > logs/finetune_full/fold5.log 2>&1 &
PID5=$!
...
echo ""
echo "监控命令:"
echo "  - 查看所有日志: tail -f logs/finetune_full/*.log"
echo "  - 查看 Fold 1: tail -f logs/finetune_full/fold1.log"
echo "  - 查看 GPU 使用: watch -n 1 nvidia-smi"
echo ""

# 等待所有任务完成
echo "等待训练完成（预计 10-12 小时）..."
...
for fold in {1..5}; do
    LATEST_RUN=$(ls -td runs/ema_iemocap_finetune_full_iemocap_fold${fold}_* 2>/dev/null | head -1)
...
for fold in range(1, 6):
    pattern = f"runs/ema_iemocap_finetune_full_iemocap_fold{fold}_*"
...
if results:
    ...
    if avg_v >= 0.68:
        print("✓✓ 超额完成目标 (CCC-V ≥ 0.68)!")
    elif avg_v >= 0.64:
        print("✓ 达到预期目标 (CCC-V ≥ 0.64)")
    else:
        print(f"✗ 未达到目标 (CCC-V {avg_v:.4f} < 0.64)")
else:
    print("未找到结果文件")
EOF

echo ""
echo "请更新实验记录: runs/experiment_comparison.md"
